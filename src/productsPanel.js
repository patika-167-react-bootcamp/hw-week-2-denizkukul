...
class ProductList extends Component {
  template() {
    return (`
      ...
      ${this.products.value.map(product => `<li>${new ProductListItem({ product, cart: this.cart, currentUser: this.currentUser, parentComponent: this }).target()}</li>`).join("")}
      </ul>
    `)
  }
}
...
class ProductListItem extends Component {
  addToCart() {
    ...
    if (currentItemIndex > -1) {
      let newItem = { ...currentItem, amount: (currentItem.amount + 1) }
      cartState.splice(currentItemIndex, 1, newItem);
